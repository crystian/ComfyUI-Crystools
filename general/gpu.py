...
class CGPUInfo:
  ...
  def __init__(self):
    ...
    if "amd" in self.torchDevice.lower():
      try:
        rocml.smi_initialize()
        self.pyrsmiLoaded = True
      except Exception as e:
        self.pyrsmiLoaded = False
    ...
    # for AMD GPUs
      ...
      self.cuda = True
      #logger.info(f'NVIDIA Driver: {pynvml.nvmlSystemGetDriverVersion()}')
      #logger.info(f'AMD Driver: {rocml.smi_get_kernel_version}') Not Available
    else:
  ...
  def getStatus(self):
    ...
    if self.cudaDevice == 'cpu':
      ...
      if self.cuda and self.cudaAvailable:
